...
def create_proxy_matrix():
    p, w = proxy_matrix()
    save_proxy_matrix(p, PROXY_PATH.joinpath("data.dat"))
    save_proxy_weights(w, PROXY_PATH.joinpath("weights.dat"))


def proxy_matrix():
    # Return a proxy data matrix with year in the first column.
    datalist_path = CONFIG_PATH.joinpath("datalist.dat")
...
def reconstructed_svd(eof_list):
    # Load proxy matrix.
    p = np.genfromtxt(PROXY_PATH.joinpath("data.dat"))
    w = np.genfromtxt(PROXY_PATH.joinpath("weights.dat"))
    ...
    for step in sorted(reconstruction_steps()):
        index = p[:, 0] >= step
        p_step = p[index, :]
...
def analyze_reconstruction(eof_list):
    ...
    path = VALIDATION_PATH.joinpath(dirname)
    path.mkdir(exist_ok=True)
